...
record = "appointment.txt"
def appoint(patient_id, time_for_checkup, disease):
    fw = open(record, "a")
    fw.write("%1s%20s%20s\n" % (patient_id, time_for_checkup, disease))
    fw.close()

while True:
    print("1. Receptionist Login\n")
    print("2. Physician Login\n")
    print("3. Accountant Login\n")
    Login = input("Please choose '1' for receptionist login '2' for physician login and '3' for accountant login\n")
    print(Login)
    ...
    if Login == "1":
        ...
        if (username, password) == ("a","a"):
            ...
            if choose_patient == "1":
                ...
                input("\nPlease press ENTER to return back\n")
        ...
        print("Please fulfill the following requirements")
        ...
        print("\nPlease check the appointment detail\n")

        res = open("appointment.txt")
        extract = input("Please input patient ID\n")
        for each_line in res:
            (patient_id, time_for_checkup, disease) = each_line.split()
            if (patient_id == extract):
                print(patient_id, time_for_checkup, disease)
        res.close()
